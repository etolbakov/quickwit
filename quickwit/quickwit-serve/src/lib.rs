...
use quickwit_metastore::{
    ListIndexesMetadataRequestExt, ListIndexesResponseExt, MetastoreResolver,
};
...
pub async fn serve_quickwit(
    ...
    let metastore_client: MetastoreServiceClient =
        if let Some(metastore_server) = &metastore_server_opt {
            ...
            let metastore_client = MetastoreServiceClient::from_channel(balance_channel);
            let retry_layer = RetryLayer::new(RetryPolicy::default());
            // TODO: add parameter to configure timeout.
            // let timeout_layer = TimeoutLayer::new(Duration::from_secs(120)).layer(retry_layer);
    ...
    // let metastore_through_control_plane: MetastoreServiceClient =
    // Arc::new(ControlPlaneMetastore::new(     control_plane_service.clone(),
    //     metastore_client.clone(),
    // ));
    let metastore_through_control_plane = metastore_client.clone();
...
async fn node_readiness_reporting_task(
    ...
    loop {
        ...
        //     }
        //     Err(error) => {
        //         warn!(metastore_uri=%metastore.uri(), error=?error, "Metastore service is
        // unavailable.");         false
        //     }
        // };
...
mod tests {
    ...
    async fn test_check_cluster_configuration() {
        ...
        metastore.expect_list_indexes_metadatas().return_once(|_| {
            Ok(
