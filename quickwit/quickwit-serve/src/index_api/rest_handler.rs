...
use quickwit_metastore::{
    IndexMetadata, IndexMetadataResponseExt, ListIndexesMetadataRequestExt, ListIndexesResponseExt,
    ListSplitsQuery, ListSplitsRequestExt, ListSplitsResponseExt, Split, SplitInfo, SplitState,
};
use quickwit_proto::metastore::{
    DeleteSourceRequest, EntityKind, IndexMetadataRequest, ListIndexesMetadatasRequest,
...
async fn describe_index(
    ...
    let index_stats = IndexStats {
        ...
        num_published_splits: 0, // published_splits.len(),
...
mod tests {
    ...
    use quickwit_proto::metastore::{
        EmptyResponse, IndexMetadataResponse, ListIndexesMetadatasResponse, ListSplitsResponse,
    ...
    async fn test_create_index_with_bad_config() -> anyhow::Result<()> {
        let metastore = MockMetastoreService::new();
        let index_service = IndexService::new(
            MetastoreServiceClient::new(metastore),
            StorageResolver::unconfigured(),
        );
